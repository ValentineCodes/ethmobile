      ...
      try {
        ...
        if (response.ok) {
          ...
          if (apiResponse.error) {
            throw new Error(
              `API Error: ${apiResponse.error} - ${apiResponse.details || ""}`,
            );
          }
          ...
          if (userData) {
            const profileData: ProfileData = {
              fid,
              username:
                userData.username || fallbackData?.username || `user${fid}`,
              displayName:
                ...
                `User ${fid}`,
              pfpUrl:
                userData.pfp?.url || fallbackData?.pfpUrl || "/images/logo.png",
          ...
          throw new Error(
            `API response not ok: ${response.status} - ${errorData.error || "Unknown error"}`,
          );
        }
      } catch (error) {
